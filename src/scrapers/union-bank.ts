...
const NO_TRANSACTION_IN_DATE_RANGE_TEXT =
  'לא קיימות תנועות מתאימות על פי הסינון שהוגדר';
...
function editLastTransactionDesc(
  txnRow: TransactionsTr,
...
async function getTransactionsTableHeaders(page: Page, tableTypeId: string) {
  ...
  const headersObjs = await pageEvalAll(
    page,
    `#WorkSpaceBox #${tableTypeId} tr[class='header'] th`,
    null,
    (ths) => {
...
async function extractTransactionsFromTable(
  ...
  const transactionsTableHeaders = await getTransactionsTableHeaders(
    page,
    tableTypeId,
  );
...
async function chooseAccount(page: Page, accountId: string) {
  const hasDropDownList = await elementPresentOnPage(
    page,
